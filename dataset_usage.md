...
Paths:
- Intermediate (no motor samples): `synthetic_datasets/intermediate/<scenario>_<geometry>.jsonl`
- Final (with motor samples in degrees): `synthetic_datasets/final/<scenario>_<geometry>.jsonl`
...
- `motor_samples`: generated motor positions in degrees (360° per rotation), matching `real_xyz` order; five columns for HP5, etc. Empty in intermediate files but present in `final/` together with `config_used`.
...
Regenerating:
1) `python build_synthetic_datasets.py` (writes intermediates)
2) `for f in synthetic_datasets/intermediate/*.jsonl; do base=$(basename "$f"); python generate_synthetic_data.py "$f" -o synthetic_datasets/final/$base; done`
